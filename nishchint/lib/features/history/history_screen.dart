...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Future<void> _exportReport() async {
    ...
    try {
      final api = ApiService();
      final fromStr = range.start.toIso8601String().substring(0, 10);
      final toStr = range.end.toIso8601String().substring(0, 10);
      
      final response = await api.get(
        '${ApiConstants.doctorReport}?patient_id=${patient.id}&date_from=$fromStr&date_to=$toStr',
      );

      if (response != null) {
        final pdfFile = await PdfReportService.generate(reportData: response);
        
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Report ready! Sharing...')),
        );

        await Share.shareXFiles(
          [XFile(pdfFile.path)],
          text: 'Nishchint Medicine Report - ${patient.name}',
        );
      }
    } catch (e) {
      if (!mounted) return;
